  ...
  describe ' Model.User' do
    ...
    context 'validates field' do
      ...
      it 'validates user already exists' do
        user_new = User.new(username: 'eric')
        user_new.valid?
    ...
    context 'username must include the message' do
      ...
      it 'length between 4 to 20 characters' do
        users(:one).username = '123'
        users(:one).valid?
        expect(users(:one).errors[:username]).to include('Username length must be between 4 to 20 characters')
      end
    end
    ...
    context 'username must include the message' do
      ...
      it 'must be alphanumeric' do
        users(:one).username = ' eric22123123'
        users(:one).valid?
